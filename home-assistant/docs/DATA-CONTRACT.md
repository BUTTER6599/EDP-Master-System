...
**Tab *names* are not confirmed.** The export carries header rows but not
sheet names, so the names in `TABS` in `Code.gs` are the ones from the
brief, matched to headers by content. Run `?list=1` against the deployed
bridge to get the real names and correct any mismatch. A wrong name is not
silent: that tab reports `BLOCKED / Tab not found` on its card and in
...
| `bill_id` | public | |
| `due_date` | public | |
| `status` | public | `PAST DUE`/`SUSPENDED` render red |
| `priority` | public | |
...
`bill_name` is private deliberately. This tab carries a Blue Cross Blue
Shield line — a bill *name* alone discloses health coverage, so the name
is withheld at public scope even though it is not a number. A public
display gets counts, due dates and statuses only.
...
### INVENTORY / REPAIRS / PAYROLL · status BLOCKED

Configured but not yet served, because the tab names are the least certain
of the set:

- **INVENTORY** — header found (`item_id, sku_id, category, brand, model,
  serial, condition, list_price, cost_basis, …`) but the tab name is a
  guess.
- **REPAIRS** — `REPAIR_PARTS` holds parts consumed per repair, not the
  open-ticket pipeline. The ticket pipeline (`ticket_id, current_stage,
  customer_name, …`) lives in a different tab, and its name is unknown.
- **PAYROLL** — every field is `private`; there is no public projection.
  At public scope the bridge returns zero rows, not empty objects, so a
  public caller cannot even count them.

Run `?list=1` to resolve all three.
